...
const ListItem = ({ location, showOpeningHours = true }: ListItemProps) => {
    ...
    return (
        <div className={classNamesTailwind('hover:bg-gray-50 hover:cursor-pointer', listItem?.wrapper)} onClick={() => handleOnItemClick()}>
            <div className={classNamesTailwind('flex items-center text-sm text-gray-500 px-4 py-4 overflow-hidden', listItem?.item)}>
                {showDistance && (
                    <span
                    ...
                    >
                        <span className={classNamesTailwind('font-bold', listItem?.distanceKm)}>{location.distance?.toFixed(1)}</span>
                        <span className={listItem?.distanceKmLabel}>km</span>
                    </span>
                )}
                <div className={classNamesTailwind('min-w-0 flex-1')}>
                    <div className="flex flex-col text-sm">
                        <span className={classNamesTailwind('truncate font-medium text-indigo-600 block', listItem?.title)}>
                            {location.title}
                        </span>
                        <address className={classNamesTailwind('font-normal text-gray-500 not-italic', listItem?.street)}>
                            {location.street}, {location.city}
                        </address>
                    </div>
                    {showOpeningHours && location.openingHours && <OpeningHoursSlot openingHours={location.openingHours} />}
                </div>
            </div>
        </div>
    );
